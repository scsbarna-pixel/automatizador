...
class EventsManager(QDialog):
    ...
    def setup(self):
        ...
        b_close = QPushButton("Cerrar Gestor")
    ...
    def load_from_file(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Abrir Programación", "", "JSON (*.json)")
        if file_path:
            try:
                with open(file_path, 'r') as f:
                    self.events = json.load(f)
                self.save_db() # Obliga al reloj interno a actualizarse
                self.model = EventsTableModel(self.events)
                self.table.setModel(self.model)
                self.table.setItemDelegate(EventDelegate(self.table))
                QMessageBox.information(self, "Cargado", f"Cargado desde:\n{os.path.basename(file_path)}")
            except Exception as e:
                QMessageBox.critical(self, "Error", f"No se pudo cargar: {e}")

    def save_as_file(self):
        file_path, _ = QFileDialog.getSaveFileName(self, "Guardar Como...", "", "JSON (*.json)")
        if file_path:
            try:
                with open(file_path, 'w') as f:
                    json.dump(self.events, f)
                self.save_db() # Obliga al reloj interno a actualizarse también
                QMessageBox.information(self, "Guardado", f"Programación guardada en:\n{os.path.basename(file_path)}")
            except Exception as e:
